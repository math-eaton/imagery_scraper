...
for chunk_idx, df_chunk in tqdm(enumerate(pd.read_csv('data/FM_service_contour_current.csv', chunksize=chunksize, nrows=5000)), total=total_chunks, desc='Total CSV'):
    ...
    for _, row in tqdm(df_chunk.iterrows(), total=chunk_rows, desc='Processing Chunk', leave=False):
        ...
        if simplified_locations:
            ...
            folium.PolyLine(locations=simplified_locations, color='blue', weight=2.5, opacity=1).add_to(m)
